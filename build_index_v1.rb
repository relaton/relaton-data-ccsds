...
# Rebuild `index-v1.yaml` from the curated `data/` tree.
#
# `index-v1` is the index consumed by the *released* relaton-ccsds gem line,
# which parses identifiers with pubid v1 (`pubid-ccsds`). The crawler proper
# runs the monorepo gem, which loads pubid v2 (`pubid`) and writes `index-v2`.
# pubid v1 and pubid v2 both define `Pubid::Ccsds::Identifier` and cannot be
# loaded in the same process, so this script runs standalone (spawned by
# `crawler.rb` in a clean bundler env) with its own pubid-v1 dependency set.
#
...
index = Relaton::Index::Type.new(
  :ccsds, nil, INDEX_FILE, nil, Pubid::Ccsds::Identifier
)
...
Dir["data/**/*.yaml"].each do |file|
  ...
  index.add_or_update(Pubid::Ccsds::Identifier.parse(content), file)
  count += 1
